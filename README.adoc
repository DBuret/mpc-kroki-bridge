...
a|An implementation of the Model Context Protocol (MCP) that gives AI agents  access to https://kroki.io[Kroki] to render diagrams and data visualizations. Built with Rust for speed, reliability & minimal footprint.

Unlike standard MCP servers (stdio only), this bridge runs over HTTP:

* **SSE + Hybrid Response**: Works with LM Studio and network clients
* **MPC 2025 (/mpc POST)**: Native Open WebUI integration
|===
...
admin pannel -> settings -> external tools -> + 

[horizontal]
type:: MCP streamble HTTP
URL:: http://host:port/mcp

-> click on refresh to check connection
